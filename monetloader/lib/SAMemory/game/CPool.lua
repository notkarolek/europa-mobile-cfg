...
ffi.cdef[[
	...
	typedef struct CPool
	{
		...
		bool 							field_11; // bDealWithNoMemory
	} CPool;
]]
...
shared.require 'CHeli'
shared.require 'CPlane'
shared.require 'CBmx'
shared.require 'CBoat'
shared.require 'CCutsceneObject'
shared.require 'CCopPed'

local data_type_object_size = {
	CVehicle = math.max(sizeof('CHeli'), sizeof('CBmx'), sizeof('CPlane'), sizeof('CBoat'));
	CObject = sizeof('CCutsceneObject');
	CTask = sizeof('uintptr_t') == 8 and 168 or 128; -- 128 on x32, 168 on x64
	CPed = sizeof('CCopPed');
}
