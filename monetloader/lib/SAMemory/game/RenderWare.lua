...
shared.ffi.cdef[[
	...
	typedef enum RpLightType
	{
		...
		LIGHT_TYPE_SPOT_2 = 0x82, // Soft spot
	...
	typedef struct RwCamera
	{
		...
		float                farplane;
		float                fog;
		float                unknown1; // zScale
		float                unknown2; // zShift
	...
	{
		unsigned int     unknown1; // flags
		unsigned short   startMorphTarget;
		unsigned short   endMorphTarget;
		float            unknown3; // time
		float            unknown4; // recipTime
		float            unknown5; // position
	} RpInterpolation;
	...
	typedef struct RpAtomic
	{
	  ...
	  unsigned short   unknown7; // pad
	  RwList           sectors;
	  void             *render; // pipeline
	} RpAtomic;
	...
	typedef struct RpLight
	{
		...
		float           radius;
		RwColorFloat    color;
		float           unknown1; // minusCosAngle
		RwList          sectors;
		RwListEntry     globalLights;
		unsigned short  frame;
		unsigned char   isMainLight;
		unsigned char   pad;
	} RpLight;
	...
	typedef struct RpGeometry
	{
	  RwObject             object;
	  unsigned int         flags;
	  unsigned short       unknown1; // lockedSinceLastInst
	  short                refs;
	  int                  triangles_size;
	  int                  vertices_size;
	  int                  unknown_size; // morphtargets_size
	  int                  texcoords_size;
	  RpMaterials          materials;
	  RpTriangle           *triangles;
	  RwColor              *colors;
	  RwTextureCoordinates *texcoords[8];
	  void                 *unknown2; // vertexBuffer
	  void                 *info; // mesh
	  void                 *unknown3; // repEntry
		void                 *morphTarget;
	} RpGeometry;
	...
	{
		int err1,err2; // pluginID, errorCode
	} RwError;
]]
